...
class HtmlCompressor
{
	...
	{
		if ($this->compressScript && $this->compressCode && strpos($this->content, '<textarea') === false) return;
		$lastChar = '';
		for ($i = 0; $i < strlen($this->content); $i++)
		{
			$char = $this->content{$i};
			$this->readChar($char, $lastChar);
			$lastChar = $char;
		}
	 ...
	 * @param string $char
	 */
	private function readChar($char, $lastChar)
	{
		$this->word .= $char;
		$this->part .= $char;
...
//		$this->inCorners($char, $lastChar);
	...
	private function inCorners($char, $lastChar)
	{
        if($char == '"' || $char == "'")
		{
			if($lastChar != '\\')
